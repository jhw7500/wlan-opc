...
/* Read a small text file into buf (NUL-terminated). 16 KiB covers
 * /proc/diskstats on the target (a handful of mmcblk devices); the unit
 * tests point the probe at synthetic files anyway. */
...
int opcd_fault_probe_sample(opcd_fault_probe_t *p, opcd_fault_report_t *out)
{
    ...
    if (!p->primed) {
        p->cpu_busy = busy;  p->cpu_total = total;
        p->disk_io_ms = disk;
        p->net_bytes  = net;
        p->mono_ms    = now;
        p->primed     = true;
        return 0;                              /* first call only primes */
    }

    uint64_t elapsed = now > p->mono_ms ? now - p->mono_ms : 1;
    /* a counter reset (interface re-init) clamps to 0 instead of wrapping */
    uint64_t d_busy  = (cpu_ok  && busy  >= p->cpu_busy)   ? busy  - p->cpu_busy   : 0;
    uint64_t d_total = (cpu_ok  && total >= p->cpu_total)  ? total - p->cpu_total  : 0;
    uint64_t d_disk  = (disk_ok && disk  >= p->disk_io_ms) ? disk  - p->disk_io_ms : 0;
    uint64_t d_net   = (net_ok  && net   >= p->net_bytes)  ? net   - p->net_bytes  : 0;

    opcd_fault_evaluate(p, d_busy, d_total, d_disk, elapsed, d_net,
                        net_ok ? read_link_mbps(p) : 0, out);
