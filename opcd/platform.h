...
typedef struct opcd_platform_ops {
     ...
     * code; see ids.h OPC_ERR_RADIO_APPLY / D9). On a non-zero return opcd
     * best-effort re-applies the last-good config — on a non-zero return opcd
     * invokes this hook a second time with the last-good cfg — so a partial
     * apply leaves no net change.
